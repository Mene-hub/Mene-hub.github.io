...
function gitConnection(){
	...
	get.onload = function() {
		if (get.status != 200) { // analyze HTTP status of the response
			...
			{
				if(GitRepose[i].name.toLowerCase()=="materialmessagebox"){
					var desc = GitRepose[i].description == null ? "there isn't any description" : GitRepose[i].description;
					template ="<div class='repoli'><a href='materialmessagebox.html' class='Roboto400 glowLink' style='font-size: 200%;'>" + GitRepose[i].name.toLowerCase() + "</a><p style='font-size: 100%;font-family:Cascadia;'>" + desc + "</p>";
